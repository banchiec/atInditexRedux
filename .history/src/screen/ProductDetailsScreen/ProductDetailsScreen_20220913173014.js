							...
							<Options>
								{ colors && optionColors?.values?.map((item) => {
									return(
										<SelectButton 
											key={item.name}
											nameSelector={optionColors.name} color={item.name} item={item} handleSelectionOptions={handleSelectionOptions} > {item.name} </SelectButton>)
								})}
								{ storage && optionStorage?.values?.map((item) => {
									return(
										<SelectButton
											key={item.name}
											nameSelector={optionStorage.name}
											color={item.name}
											item={item}
